...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt_entity, tgt_triplet, entity_pos, triplet_pos, so_pos,
        ...
        _tgt_entity = tgt_entity.clone().detach()
        ...
        # debias
        _tgt_sub = tgt_sub.clone().detach()
        _triplet_pos = triplet_pos.clone().detach()
        _tgt2_sub = self.cross_sub_entity(query=self.with_pos_embed(_tgt_sub, _triplet_pos),
                                         key=_tgt_entity, value=_tgt_entity)[0]
        _tgt_sub = _tgt_sub + self.dropout2_sub(_tgt2_sub)
        ...
        # debias
        _tgt_obj = tgt_obj.clone().detach()
        _tgt2_obj = self.cross_obj_entity(query=self.with_pos_embed(_tgt_obj, _triplet_pos),
                                         key=_tgt_entity, value=_tgt_entity)[0]
        _tgt_obj = _tgt_obj + self.dropout2_obj(_tgt2_obj)
